...
// the container path which will be /tmp/vpmem<location>/
//
// TODO: Consider a structure here so that we can extend for future functionality without
//       breaking the API surface.
func AddVPMEM(uvm Container, hostPath string, expose bool) (int, string, error) {
	location := -1
	containerPath := ""
	if uvm == nil {
	...
	uvmc := uvm.(*container)
	logrus.Debugf("hcsshim::AddVPMEM id:%s hostPath:%s expose:%b sv:%s", uvmc.id, hostPath, expose, uvmc.schemaVersion.String())
