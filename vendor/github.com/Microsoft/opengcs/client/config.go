...
type Config struct {
	...
	Options            hcsshim.LCOWOptions         // Configuration for the utility VM
}
...
func (config *Config) GenerateDefault(options []string) error {
	...
	var err error
	config.Options, err = hcsshim.ParseLCOWOptions(options)
	if err != nil {
		return err
	}
	...
	if config.Options.TimeoutSeconds != 0 {
		config.UvmTimeoutSeconds = config.Options.TimeoutSeconds
		return nil
	}
...
func (config *Config) Validate() error {
	...
	if config.RequestedMode == ModeRequestVhdx && config.Options.Vhdx == "" {
		return fmt.Errorf("VHDx mode must supply a VHDx")
	}
	if config.RequestedMode == ModeRequestKernelInitrd && (config.Options.KernelFile == "" || config.Options.InitrdFile == "") {
	...
	if config.RequestedMode == ModeRequestAuto || config.RequestedMode == ModeRequestVhdx {
		if _, err := os.Stat(config.Options.Vhdx); os.IsNotExist(err) {
			if config.RequestedMode == ModeRequestVhdx {
				return fmt.Errorf("VHDx '%s' not found", config.Options.Vhdx)
			}
		} else {
			config.ActualMode = ModeActualVhdx

			// Can't specify boot parameters with VHDx
			if config.Options.BootParameters != "" {
	...
	if config.Options.InitrdFile == "" || config.Options.KernelFile == "" {
	...
	if _, err := os.Stat(filepath.Join(config.Options.KirdPath, config.Options.KernelFile)); os.IsNotExist(err) {
		return fmt.Errorf("kernel '%s' not found", filepath.Join(config.Options.KirdPath, config.Options.KernelFile))
	}
	if _, err := os.Stat(filepath.Join(config.Options.KirdPath, config.Options.InitrdFile)); os.IsNotExist(err) {
		return fmt.Errorf("initrd '%s' not found", filepath.Join(config.Options.KirdPath, config.Options.InitrdFile))
	}
...
func (config *Config) StartUtilityVM() error {
	...
	if config.ActualMode == ModeActualVhdx {
		configuration.HvRuntime = &hcsshim.HvRuntime{
			ImagePath:          config.Options.Vhdx,
			BootSource:         "Vhd",
			WritableBootSource: false,
		}
	} else {
		configuration.HvRuntime = &hcsshim.HvRuntime{
			ImagePath:           config.Options.KirdPath,
			LinuxInitrdFile:     config.Options.InitrdFile,
			LinuxKernelFile:     config.Options.KernelFile,
			LinuxBootParameters: config.Options.BootParameters,
		}
	}
	...
	// JJH This should move to the new method.
